...
int main() {
    ...
    // Create FIFOs. 0666 means read/write permissions for everyone
    ...
    int fifo_cs = open(FIFO_CS, O_RDONLY);
    if (fifo_cs == -1) {
        perror("open fifo");
        return 1;
    }

    int fifo_sc = open(FIFO_SC, O_WRONLY);
    if (fifo_sc == -1) {
        perror("open fifo");
        return 1;
    }
    ...
    while (read(fifo_cs, buffer, BUF_SIZE) > 0) {
        ...
        write(fifo_sc, response, strlen(response) + 1);
    }
