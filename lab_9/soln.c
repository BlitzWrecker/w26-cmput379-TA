// This version only works for at most 2 ssh destinations. For Assignment 2, you will need to handle an arbitrary number
// of ssh destinations.
#include <arpa/inet.h>
#include <netdb.h>
...
int main(int argc, char *argv[]) {
    // Check input args
    if (argc < 5) {
        printf("usage %s port ssh_executable client_executable ssh_destination_1 (optional)ssh_destination_2\n", argv[0]);
        exit(0);
    }
    int port_number = atoi(argv[1]);    // ex. 5544
    char *ssh_executable = argv[2];     // ex. /usr/bin/ssh
    char *client_executable = argv[3];  // ex. ~/379/labs/lab9/client
    char *ssh_destinations[2] = {argv[4], NULL};  // ex. "user@remote_machine_1" "user@remote_machine_2"

    if (argc >= 6) {
        ssh_destinations[1] = argv[5];
    }
    ...
    for (int i = 0; i < 2; i++) {
        char *ssh_destination = ssh_destinations[i];
        printf("Iteration %d\n", i + 1);

        if (ssh_destination == NULL) {
            printf("No more ssh destinations provided. Skipping.\n");
            continue;
        }
